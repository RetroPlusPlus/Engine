...
Three enemy craft classes cross the sea firing straight, readable bullets — an aimed bolt
(corsair), a six-bolt ring (warden), a three-bolt fan (dreadnought) — every trajectory
predictable: dodge by reading, never by luck. An abductor saucer steals whoever waits (ram its
beam, or let your crew shoot it — both foil the theft and pay a bounty), and a soul carried off
the top comes back as a mutant that hunts you. Delivering n souls at once pays escalating (slot
i pays i × 50), so the greed dial is how heavy — how slow, how armed — you dare get.
...
| Drop the last passenger | Z or numpad Enter | B (east) |
...
- **Waves:** rescue the quota and the crossing clears; the next runs denser and faster. Waiting
  souls persist between waves.
...
| `assets.{h,cpp}` | Slices the four committed indexed PNGs (game sheets + the bespoke title set), loads EVERY palette from a palette image (`loadPaletteImage`, 30 of them), builds the shared animation clips. |
| `feel.{h,cpp}` | Presentation state the sim never sees: popups, the death shake, the beam/glow breaths, the CROSSING round card, the animation cursors, pooled booms. |
| `audio.{h,cpp}` | Registers the seven SM83 chiptune SFX and cues them per `GameEvent`. |
...
|---|---|---|
| Palette images | all 30 palettes are 16×1 RGBA PNGs via `loadPaletteImage` — no colour table in C++; the floating-text backgrounds are alpha-0 ENTRIES | [tiles-and-colour](../../docs/guide/tiles-and-colour.md) |
