...
enum SfxSlot : std::size_t {
    ...
    SFX_PICKUP,    // a colonist boards (also voices a deliberate Drop)
    SFX_BANK,      // the deck delivered — the rising sweep (also voices a crossing cleared)
    SFX_ALARM,     // the two-note klaxon: a beam lit OR a mutant arrived
...
FerrymanAudio::FerrymanAudio() {
    ...
    sfx_[SFX_ALARM]  = lib.registerAudio("examples/ferryman/assets/sfx/alarm.asm",  AudioType::Sfx, Isa::Sm83, AssetPolicy::Embed);
...
void FerrymanAudio::onEvent(GameEventKind kind) {
    // The 11 events → 7 voices. The EMOTION picks the sound, not the mechanism: the two boarding
    // moments share the blip, the two triumphs share the sweep, the two threats share the
    // klaxon, and the splat voices both the kill and the loss that caused it. Enemy fire is
    // deliberately UNVOICED — a toned-down bullet hell telegraphs visually, and voicing every
    // hostile bolt would bury the cues that matter.
    std::size_t slot;
    switch (kind) {
        case GameEventKind::CargoFire:    slot = SFX_PEW; break;
        case GameEventKind::Pickup:
        case GameEventKind::Drop:         slot = SFX_PICKUP; break;
        case GameEventKind::Bank:
        case GameEventKind::WaveClear:    slot = SFX_BANK; break;
        case GameEventKind::BeamLock:
        case GameEventKind::MutantSpawn:  slot = SFX_ALARM; break;
