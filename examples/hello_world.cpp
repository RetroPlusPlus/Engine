// The minimal complete GBCPP-Engine program — the companion to docs/guide/getting-started.md.
// It opens a window and draws a scrolling, indexed-colour tile background you steer with the d-pad,
// using only hand-built art (no asset files), so it is the smallest thing that compiles, links, and
// runs against the live engine. The getting-started guide walks through it block by block.
//
// Like the window demo, this is built on every CI platform (so it keeps compiling against the live
// SdlPlatform + Renderer) but never run in CI, which has no display. Run it on a dev machine.
...
int main() {
    ...
    // 4. A tilemap: a 32×32 grid checkerboarding the two tiles. Kept alive for the program's life
    //    (the layer holds a span into it).
    ...
    for (int y = 0; y < kMapH; ++y) {
        for (int x = 0; x < kMapW; ++x) {
            cells[static_cast<std::size_t>(y) * kMapW + x].tile =
                static_cast<std::uint16_t>((x + y) % 2);  // alternate tile 0 / tile 1
        }
    }
