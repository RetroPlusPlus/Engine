...
// ENG-4.D.1 — the internal test seam for driving AudioSystem production SYNCHRONOUSLY, with the
// production thread suppressed.
//
// Production normally runs autonomously on its own thread (eng-4.d.1-audio-production-thread.md), so the
// game just cues with play()/stop() and never steps anything — there is deliberately no public stepping
// method. But device-free tests need DETERMINISTIC production: drive one pass, assert the exact buffer
// level. This seam gives them that without polluting the game-facing surface — `makeManual` builds an
// AudioSystem whose production thread is NOT started (play()/stop() apply their cue inline instead of
// marshaling), and `step` / `stepDriverRaw` drive it by hand on the calling thread. Reachable only
// through this internal header (a friend of AudioSystem); never in include/retropp/.
//
...
// tests get the same deterministic, synchronous production the removed tick() used to give.
struct AudioSystemTestAccess {
    ...
    // frame-quantized refill-to-target produce pass and the auto-close check. This is the synchronous
    // equivalent of the old tick() — the migrated AudioSystem tests use it.
    ...
    // (the golden byte-identical gate compares two granularities over the same total cycles).
