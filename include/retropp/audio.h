...
// console-specific synthesis lives behind the per-system VM backend; what crosses HERE is only finished
// stereo PCM.
//
// The audio production thread pushes finished frames; an AudioSink drains them to the speakers on its own
// audio thread. The two sides meet through one single-producer / single-consumer hand-off (see
// src/audio/ring_buffer.h); a second SPSC hand-off (src/audio/cue_queue.h) carries the game's
// play()/stop() cues to that production thread. The sim/render loop itself stays single-threaded.
//
// What this is: just the sink + the PCM frame format the whole chain speaks. The cue surface a game drives
// is the AudioSystem (play/stop) over the AudioLibrary catalog.
//
// No machine or SDL type crosses this boundary — a frame is two int16 samples, a sink is open/pull/close.
// The production sink is SdlAudioSink (owned by SdlPlatform); tests drive a capture sink.
...
// The one rate the whole chain runs at. The active backend's synthesizer (the Game Boy APU today) is set
// to output at EXACTLY the sink rate, so it resamples internally — the three-rate-alignment problem
// collapses to one number. 48 kHz stereo is the default.
