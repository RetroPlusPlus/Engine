...
// The result of loading + slicing an atlas image: the uploaded atlas handle plus the carved sub-asset
// slots in read order. `manifest[i]` is the i-th carved asset's slot (its top-left atlas cell +
// dimensions) — feed slot.tile to a TileCell::tile / Sprite::tile and slot.dimensions to a Sprite::size.
// It carries the AtlasId (the uploaded-atlas handle) beside the sliced slots (pure geometry from
// sliceLayout). The atlas uploads once — re-slicing for a different order/kind is a pure sliceLayout call
// against the same AtlasId, no re-upload.
//
// A pure data type: it depends only on the atlas handle + slot geometry, so a consumer that just needs a
// sheet's layout (e.g. an AnimationFrame resolving a slot index) includes this header alone, without the
// GPU renderer. Renderer::loadAtlas (renderer.h) produces one; renderer.h re-exports this type by
