...
| Built-in effect library (`RowDisplacement` wave, `Ripple` droplet, `ColorFill` solid colour fill / drawn line, `Gleam` diagonal sheen sweep, `ColorSaturation` colour drain — name the kind, the engine owns the shader) | available | draw-state.md |
