...
| [sprites.md](sprites.md) | The `Sprite` drawable in full: the required `key` reconciliation identity and keying rules, placement (`x`/`y`, `origin` vs `pivot`, `center(Space)`), the arbitrary-but-8-aligned `size` read + console presets, art (`atlas`/`tile`/`palette`), texture flips + 90° `rotation`, per-sprite `alpha` and `blend`, the geometric `transform`, the `anchors` articulation resolvers (`anchor(k, Space)` / `toLayer`), the effect carrier (`effects` chain + `regions` — whole-silhouette, quad-space, and Below-scope scene lensing), and what interpolates vs snaps. | `draw_state.h`, `geometry.h` |
