...
| [sprite-path.md](sprite-path.md) | Driving a whole sprite along a curve: `SpritePathNode` (a movement spec + pacing + a facing policy + rotation / scale tween tracks + an animation), the `SpritePath` cursor that composes them off one clock, and the `applyTo(Sprite&)` write (position, transform, frame art, flip) beside the raw composed sample. The orchestrator over the path walker. | `sprite_path.h` |
...
| Sprite paths (`SpritePath` cursor composing movement + rotation / scale tween tracks + an animation + a facing policy off one clock; `applyTo(Sprite&)` write plus the raw sample) | available | sprite-path.md |
