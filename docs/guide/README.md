...
| [sprites.md](sprites.md) | The `Sprite` drawable in full: the required `key` reconciliation identity and keying rules, placement (`x`/`y`, `origin` vs `pivot`, `center()`), the arbitrary-but-8-aligned `size` read + console presets, art (`atlas`/`tile`/`palette`), texture flips + 90° `rotation`, per-sprite `alpha` and `blend`, the geometric `transform`, the `anchors` articulation resolvers (`anchorQuad` / `anchorLayer` / `toLayer`), the effect carrier (`effects` chain + `regions` — whole-silhouette, quad-space, and Below-scope scene lensing), and what interpolates vs snaps. | `draw_state.h`, `geometry.h` |
| [anchors-and-articulation.md](anchors-and-articulation.md) | Multi-part figures from plain sprites: anchors (named art-space points a sprite publishes; `anchorQuad` / `anchorLayer` resolvers, label-or-index addressing, flips/rotation mirror them with the art), origin & pivot (`Sprite::origin` — the point `x/y` place; `Sprite::pivot` — the transform spin centre; set both to one point for a joint), `Sprite::center()`, attachment by re-feeding a parent's anchor into a child's position each tick, per-sprite `z` (non-unique within-layer stacking, stable ties), and pinning a `Curve` / any point consumer to an anchor. | `draw_state.h` |
...
| Sprites (placement, art, `alpha`/`blend`, transform, anchors, the `effects`/`regions` carrier + Below-scope lensing) | available | sprites.md |
