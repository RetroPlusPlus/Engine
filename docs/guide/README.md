...
| [anchors-and-articulation.md](anchors-and-articulation.md) | Multi-part figures from plain sprites: anchors (named art-space points a sprite publishes; `anchorLocal` / `anchorAt` resolvers, label-or-index addressing, flips/rotation mirror them with the art), the pivot (`Sprite::pivot` — the point `x/y` place AND the transform centre, one point so placement and hinge agree), attachment by re-feeding a parent's anchor into a child's position each tick, per-sprite `z` (non-unique within-layer stacking, stable ties), and pinning a `Curve` / any point consumer to an anchor. | `draw_state.h` |
...
| Sprite anchors + pivot placement + per-sprite z-order (published points, pivot-on-anchor attachment, within-layer stacking) | available | anchors-and-articulation.md |
