...
| [how-to.md](how-to.md) | Task recipes — scroll, walk-behind, HUD, sprites, fades, recolour, PNG loading, atlas slicing, animation, tweening, menus, a draggable title bar on a chromeless window, and the retained-vs-rebuilt frame patterns. |
...
| Built-in effect library (`RowDisplacement` wave, `Ripple` droplet, `ColorFill` solid colour fill / drawn line, `Gleam` diagonal sheen sweep, `ColorSaturation` colour drain, `Bloom` glow halo, `Glow` authored-colour aura — name the kind, the engine owns the shader) | available | draw-state.md |
