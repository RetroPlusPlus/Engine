...
| Region-confined screen-space effects (a `Region` owns a shape — polygon, curve, or SDF — and the effects applied inside it; layers and the frame own a list of them) | available | draw-state.md |
| Stencil see-through primitive (make a layer transparent inside or outside its shape, feathered — reveal the layers below or the backdrop; each side an optional effect-able region) | available | draw-state.md |
